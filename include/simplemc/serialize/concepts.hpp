 ...
 * `const T&`. The requirements for `S` to be a serializer are the following:
 *
...
concept serializer = requires(S& s, const S& cs, std::string_view key, const T& value) {
    { s[key] } -> std::same_as<S>;
 ...
 * type `T`. The requirements for the pair `S` to be a deserializer are the following:
 *
 ...
 */
template <class S, class T = int>
concept deserializer = requires(const S& cs, std::string_view key, T& value) {
    { cs[key] } -> std::same_as<S>;
    { cs.has(key) } -> std::convertible_to<bool>;
    { cs.load_at(key, value) } -> std::same_as<S>;
};
