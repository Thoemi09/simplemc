...
TEST_F(SimplemcAccsMPIStochasticProcess, MeanAccumulator) {
    ...
    // double standard
    simplemc_mpi_collect(comm, macc_std_d);
    EXPECT_EQ(macc_std_d.count(), steps);
    check_near(macc_std_d.mdata(), exp_macc_std_d.mdata(), tol);

    // double welford
    simplemc_mpi_collect(comm, macc_wel_d);
    EXPECT_EQ(macc_wel_d.count(), steps);
    check_near(macc_wel_d.mdata(), exp_macc_wel_d.mdata(), tol);

    // complex standard
    simplemc_mpi_collect(comm, macc_std_c);
    EXPECT_EQ(macc_std_c.count(), steps);
    check_near(macc_std_c.mdata(), exp_macc_std_c.mdata(), tol);

    // complex welford
    simplemc_mpi_collect(comm, macc_wel_c);
    EXPECT_EQ(macc_wel_c.count(), steps);
    check_near(macc_wel_c.mdata(), exp_macc_wel_c.mdata(), tol);
}
...
TEST_F(SimplemcAccsMPIStochasticProcess, VarianceAccumulator) {
    ...
    // double standard
    simplemc_mpi_collect(comm, vacc_std_d);
    EXPECT_EQ(vacc_std_d.count(), steps);
    check_near(vacc_std_d.mdata(), exp_vacc_std_d.mdata(), tol);
    check_near(vacc_std_d.cdata(), exp_vacc_std_d.cdata(), tol);

    // double welford
    simplemc_mpi_collect(comm, vacc_wel_d);
    EXPECT_EQ(vacc_wel_d.count(), steps);
    check_near(vacc_wel_d.mdata(), exp_vacc_wel_d.mdata(), tol);
    check_near(vacc_wel_d.cdata(), exp_vacc_wel_d.cdata(), tol);

    // complex standard
    simplemc_mpi_collect(comm, vacc_std_c);
    EXPECT_EQ(vacc_std_c.count(), steps);
    check_near(vacc_std_c.mdata(), exp_vacc_std_c.mdata(), tol);
    check_near(vacc_std_c.rdata(), exp_vacc_std_c.rdata(), tol);
    check_near(vacc_std_c.idata(), exp_vacc_std_c.idata(), tol);
    check_near(vacc_std_c.cdata(), exp_vacc_std_c.cdata(), tol);

    // complex welford
    simplemc_mpi_collect(comm, vacc_wel_c);
    EXPECT_EQ(vacc_wel_c.count(), steps);
    check_near(vacc_wel_c.mdata(), exp_vacc_wel_c.mdata(), tol);
    check_near(vacc_wel_c.rdata(), exp_vacc_wel_c.rdata(), tol);
    check_near(vacc_wel_c.idata(), exp_vacc_wel_c.idata(), tol);
    check_near(vacc_wel_c.cdata(), exp_vacc_wel_c.cdata(), tol);
}
...
TEST_F(SimplemcAccsMPIStochasticProcess, CovarianceAccumulator) {
    ...
    // double standard
    simplemc_mpi_collect(comm, cacc_std_d);
    EXPECT_EQ(cacc_std_d.count(), steps);
    check_near(cacc_std_d.mdata(), exp_cacc_std_d.mdata(), tol);
    check_near(make_span(cacc_std_d.cdata()), make_span(exp_cacc_std_d.cdata()), tol);

    // double welford
    simplemc_mpi_collect(comm, cacc_wel_d);
    EXPECT_EQ(cacc_wel_d.count(), steps);
    check_near(cacc_wel_d.mdata(), exp_cacc_wel_d.mdata(), tol);
    check_near(make_span(cacc_wel_d.cdata()), make_span(exp_cacc_wel_d.cdata()), tol);

    // complex standard
    simplemc_mpi_collect(comm, cacc_std_c);
    EXPECT_EQ(cacc_std_c.count(), steps);
    check_near(cacc_std_c.mdata(), exp_cacc_std_c.mdata(), tol);
    check_near(make_span(cacc_std_c.rdata()), make_span(exp_cacc_std_c.rdata()), tol);
    check_near(make_span(cacc_std_c.idata()), make_span(exp_cacc_std_c.idata()), tol);
    check_near(make_span(cacc_std_c.cdata()), make_span(exp_cacc_std_c.cdata()), tol);

    // complex welford
    simplemc_mpi_collect(comm, cacc_wel_c);
    EXPECT_EQ(cacc_wel_c.count(), steps);
    check_near(cacc_wel_c.mdata(), exp_cacc_wel_c.mdata(), tol);
    check_near(make_span(cacc_wel_c.rdata()), make_span(exp_cacc_wel_c.rdata()), tol);
    check_near(make_span(cacc_wel_c.idata()), make_span(exp_cacc_wel_c.idata()), tol);
    check_near(make_span(cacc_wel_c.cdata()), make_span(exp_cacc_wel_c.cdata()), tol);
}
...
TEST_F(SimplemcAccsMPIStochasticProcess, BlockAccumulator) {
    ...
    // double welford
    simplemc_mpi_collect(comm, blacc_wel_d);
    EXPECT_EQ(blacc_wel_d.total_count(), steps);
    check_near(blacc_wel_d.accumulator().mdata(), exp_blacc_wel_d.accumulator().mdata(), tol);
    check_near(blacc_wel_d.accumulator().cdata(), exp_blacc_wel_d.accumulator().cdata(), tol);
}
